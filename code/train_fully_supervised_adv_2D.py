...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            pseudo_supervision = torch.argmax(outputs, dim=1, keepdim=True).type(torch.FloatTensor).cuda()


            dis_output = dis_main(pseudo_supervision)
            g_loss = adv_loss(dis_output,REAL_LABEL)
            g_loss.backward()
            optimizer.step()
            ...
            # print(dis_real.shape,REAL_LABEL.shape)
            real_loss = adv_loss(dis_real,REAL_LABEL)
            fake_loss = adv_loss(dis_fake_detach,FAKE_LABEL)
            ...
            # # plt.show()
            # plt.subplot(232)
            # plt.imshow(volume_batch.cpu().data[0,0])#, cmap='Greys_r') # 显示图片
            # plt.subplot(233)
            # plt.imshow(real_mask.cpu().data[0,0])
            # # plt.subplot(224)
            # # plt.imshow(np.transpose(mask2,(1,2,0)))
            # plt.subplot(234)
            # plt.imshow(pseudo_supervision.cpu().data[0,0])
            # # plt.subplot(235)
            # # plt.imshow(torch.argmax(
            ...
            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, g_loss: %f, d_loss: %f' %
                (iter_num, loss.item(), loss_ce.item(),g_loss.item(),d_loss.item()))
