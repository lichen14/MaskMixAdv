...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # outputs1_hard = torch.argmax(outputs_soft1, dim=1, keepdim=False)
            # outputs2_hard = torch.argmax(outputs_soft2, dim=1, keepdim=False)

            # plt.subplot(231)
            # plt.imshow(outputs1_hard.cpu().data[0])
            # # # plt.show()
            # plt.subplot(232)
        ...
        if iter_num >= max_iterations:
            iterator.close()
            save_mode_path = os.path.join(
                snapshot_path, 'iter_' + str(max_iterations) + '.pth')
            torch.save(model.state_dict(), save_mode_path)
            logging.info("save model to {}".format(save_mode_path))
            break
    logging.info(
