...
parser.add_argument('--model', type=str,
                    default='unet_mae_cct_0117', help='model_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            plt.subplot(231)
            plt.imshow(np.transpose(t.cpu().data,(1,2,0)))
            plt.subplot(232)
            plt.imshow(mm.cpu().data)
            plt.subplot(233)
            plt.imshow(real_mask.cpu().data[0,0])
            plt.show()
            ...
            pseudo_supervision = torch.argmax(
                (beta * outputs_soft1.detach() + (1.0-beta) * outputs_soft2.detach()), dim=1, keepdim=False).type(torch.FloatTensor).cuda()
                ...
                for i in range(pseudo_supervision.shape[0]):
                    ...
                    for j in range(1,args.num_classes):
                        ...
                        # print(tmp.shape,tmp)
                            ...
                            zeros[i,labels==max_area_index]=j
            ...
            dis_out_main = dis_main(pseudo_supervision.unsqueeze(1))   #softmax得到0-1的概率prob，prob_2_entropy转化为self-information I_x
            ...
            pseudo_output = beta * outputs_soft1 + (1.0-beta) * outputs_soft2
            pseudo_supervision2 = torch.argmax(pseudo_output, dim=1, keepdim=True).type(torch.FloatTensor).cuda()

            dis_fake_detach = dis_main(pseudo_supervision2.detach())
            ...
            if iter_num % 20 == 0:
                ...
                writer.add_image('train/zeros',
                                 zeros[1, ...].unsqueeze(0) * 50, iter_num)
