...
from networks.net_factory import net_factory,get_fc_discriminator
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch, label_batch, real_mask = volume_batch.cuda(), label_batch.cuda(), real_mask.type(torch.FloatTensor).cuda()
            ...
            pseudo_supervision = torch.argmax(pseudo_output, dim=1, keepdim=False)
            dis_out_main = dis_main(pseudo_supervision.float().unsqueeze(1))   #softmax得到0-1的概率prob，prob_2_entropy转化为self-information I_x
            # print('dis_out_main',dis_out_main)    #[1, 1, 8, 8]
            loss_adv_trg_main = bce_loss(dis_out_main, REAL_LABEL)
            ...
            lamda = epoch_num * 0.01 if epoch_num >100 else 0
            loss = lamda* loss_adv_trg_main + loss_ce * 0.5 + loss_pse_sup + 10 * loss_mae         #Eq.(4) L_total lamda=0.5
            loss.backward()
            optimizer.step()
            ...
            real_mask = real_mask.unsqueeze(1).detach()    #不会对新的pred_src_main求梯度
            dis_out_main1 = dis_main(real_mask)
            # print(dis_out_main1)
            loss_dis_main1 = 0.01*bce_loss(dis_out_main1, FAKE_LABEL)
            # loss_dis_main = loss_dis_main / 2
            # loss_dis_main.backward()

            # train with target 在目标域的损失
            pseudo_supervision = pseudo_supervision.float().unsqueeze(1).detach()  #不会对新的pred_tar_main求梯度
            dis_out_main2 = dis_main(pseudo_supervision)
            loss_dis_main2 = 0.01*bce_loss(dis_out_main2, REAL_LABEL)

            loss_dis_main = loss_dis_main1 + loss_dis_main2
            ...
            writer.add_scalar('info/dis_out_main', dis_out_main[0], iter_num)
            writer.add_scalar('info/dis_out_main1', dis_out_main1[0], iter_num)
            writer.add_scalar('info/dis_out_main2', dis_out_main2[0], iter_num)
            writer.add_scalar('info/loss_mae', loss_mae, iter_num)
            writer.add_scalar('info/loss_adv_trg_main', loss_adv_trg_main, iter_num)
            writer.add_scalar('info/loss_dis_main', loss_dis_main, iter_num)
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                logging.info(
                    'iteration %d : mean_dice : %f mean_hd95 : %f' % (best_iter_num, performance, mean_hd95))
                model.train()
        ...
        if iter_num >= max_iterations:
            ...
            break
    logging.info(
    'best iteration %d : best_dice : %f best_hd95 : %f' % (iter_num, best_performance, best_mean_hd95))
    writer.close()
